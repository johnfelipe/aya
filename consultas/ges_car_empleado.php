...
$mensaje="";
	...
	if(@$_GET['buscar'])
	{
		...
		mysql_query("SET NAMES UTF8");
			...
			$query="select * from cargo where car_m like '%$c1%' AND rango=6"; //query por defaul
				...
				if(mysql_num_rows($consul)>0)
				{
					...
					while($f= $paging->fetchResultado()) 
					{
								...
								echo "<tr align=center bgcolor='#BDBDBD'>
										...
										<td><font color='white'><input type=\"button\" nane='bx' value=\"+\" onClick=\"opener.document.getElementById('text13').value='$f[car_m]+$f[car_id]\\n';window.close();\" style='cursor:pointer'></font></td>
									</tr>";
								}
								else
								{
									echo "<tr align=center>
											<td>". $f['car_m']."</td>
											<td><input type=\"button\" nane='bx' value=\"+\" onClick=\"opener.document.getElementById('text13').value='$f[car_m]+$f[car_id]\\n';window.close();\" style='cursor:pointer'></font></td>
										</tr>";

			...
			if($existe_cc > 0)
			{
				...
				while($row2=$paging->fetchResultado())
				{
						...
						echo"<tr align=center bgcolor='#BDBDBD'>
								...
								<td><input type=\"button\" nane='bx' value=\"Eliminar\"  onclick=\"open('../consultas/lista_asis_del.php?id_acta=".@$acta."&id_conse=".@$row2[nomina_conid]."&nomina_id=".@$row2[nomina_id]."&id_cargo=".@$row2[car_id]."&cargo_nom=".@$row2[car_m]."&id_per=".@$row[id_persona]."&nombre_per=".@$row2[nombres]."&ape_per=".@$row2[primer_apellido]."&nomina_consejo=".@$row[nomina_conid]."','per2','width=600, height=400, scrollbars=yes')\" style=\"cursor: pointer\" >";
							}
							else
							{
								echo"<tr align=center >
										...
										<td><font ><input type=\"button\" nane='bx' value=\"Eliminar\"onclick=\"open('../consultas/lista_asis_del.php?id_acta=".@$acta."&id_conse=".@$row2[nomina_conid]."&nomina_id=".@$row2[nomina_id]."&id_cargo=".@$row2[car_id]."&cargo_nom=".@$row2[car_m]."&id_per=".@$row[id_persona]."&nombre_per=".@$row2[nombres]."&ape_per=".@$row2[primer_apellido]."&nomina_consejo=".@$row[nomina_conid]."','per2','width=600, height=400, scrollbars=yes')\" style=\"cursor: pointer\" ></font></td>
									</tr>";

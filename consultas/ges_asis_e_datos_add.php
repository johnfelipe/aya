	  ...
	  <?php
		...
		if($id_consejo<>'')
		{
			...
			if($existe_cc > 0)
			{
					...
					<tr align='center'>
						<th>Cargo</th><th>Nombres</th><th>Primer Apellido</th><th>Periodo</th><th>agregar</th>
					<tr>";
				...
				{
					$comprobar=mysql_query("SELECT * FROM relacion_actas WHERE relacion_funcionario=$row2[nomina_id] and relacion_actaid=$acta");
						$valida=mysql_num_rows($comprobar);
					if($valida==0)
					{
						if($numero%2==0)
						{
						echo"<tr align=center bgcolor='#BDBDBD'>
								...
								<td><input type=\"button\" nane='bx' value=\"+\"  onclick=\"open('../consultas/lista_asis_add.php?id_acta=$acta&id_conse=$row2[per_consejo_id]&nomina_id=$row2[nomina_id]&id_cargo=$row2[car_id]&cargo_nom=$row2[car_m]&id_per=$row2[id_persona]&nombre_per=$row2[nombres]&ape_per=$row2[primer_apellido]&nomina_consejo=$row2[nomina_conid]','per2','width=600, height=400, scrollbars=yes')\" style=\"cursor: pointer\" >";
							}
							else
							{
								echo"<tr align=center >
										...
										<td><font ><input type=\"button\" nane='bx' value=\"+\"onclick=\"open('../consultas/lista_asis_add.php?id_acta=$acta&id_conse=$row2[per_consejo_id]&nomina_id=$row2[nomina_id]&id_cargo=$row2[car_id]&cargo_nom=$row2[car_m]&id_per=$row2[id_persona]&nombre_per=$row2[nombres]&ape_per=$row2[primer_apellido]&nomina_consejo=$row2[nomina_conid]','per2','width=600, height=400, scrollbars=yes')\" style=\"cursor: pointer\" ></font></td>
									</tr>";
					...
					else
					{
						if($numero%2==0)
						{
							echo"<tr align=center bgcolor='#BDBDBD'>
									...
									<td><font color='white'>". $row2['primer_apellido'] ."</td>
									<td><font color='white'>&nbsp;". $row2['per_consejo_des'] ." hasta ". $row2['per_consejo_has'] ."&nbsp;</td>
									<td><font color='red'>Existe en la lista</font></td>
								</tr>";
						}
						else
						{

	...
	*/
	...
	//Pie de página
		...
		function Header()
		{
			...
			$numero_real= $this->PageNo();
			
			//incio numero dinamico pagina
			$anterior=$id -1;
			
			$id2=(int)$id;
			
			//inicoconsultas libro
			$query_libro=mysql_query("SELECT act_lb  FROM actas WHERE act_id=$id");
			if(mysql_num_rows($query_libro)>0)
			{
				 $libro=mysql_fetch_array($query_libro);
			}
			// fin consultas libro
			$query_pg=mysql_query("SELECT act_np, act_Y   FROM actas WHERE act_id=$anterior AND act_lb=$libro[act_lb]");
			if(mysql_num_rows($query_pg)>0)
			{
				$row_pg=mysql_fetch_array($query_pg);
				$mxl=(int)$row_pg['act_np'];
				if($row_pg['act_Y']==25)
				{
					$numero_pga=$this->PageNo()+$mxl;
					$numero_p_sigueiente=$numero_real + $mxl;
					
				}
				else
				{
					$numero_pga=($this->PageNo()+$mxl)-1;
					$numero_p_sigueiente=($numero_real-1) + $mxl;
				}
				
				
				mysql_query("UPDATE actas SET act_np=$numero_p_sigueiente WHERE act_id=$id2");
			}
			else
			{
				
				$numero_pga=$this->PageNo();
				mysql_query("UPDATE actas SET act_np='$numero_real' WHERE act_id=$id2");
			...
			//http://ynlch.blogspot.com/search?updated-max=2011-11-07T10:30:00-06:00&max-results=25
			...
			//Número de página
			$this->Cell(20,5,$numero_pga,0,0,'R');
		$this->Ln(5);
		}
}
$q_libro_n=mysql_query("SELECT act_lb FROM actas WHERE act_id=$id");
if(mysql_num_rows($q_libro_n)>0)
{
	$libro_c=mysql_fetch_array($q_libro_n);
}

$anterior=$id -1;
$query_coor=mysql_query("SELECT act_Y FROM actas WHERE act_id=$anterior AND act_lb=$libro_c[act_lb]");
				...
				while($row_acta=mysql_fetch_array($query_acuerdos))
				{
					...
					if($valida_repro >=1)
					{
						...
						for($aa=0;$aa<=$fin; $aa++)
						{
							if($aa==0)
							{
								if($case==0)
								{
								 @$arr[]=$text1.'Comúniquese.  Acuerdo Número '.$partes[0];
								}
								else
								{
									$indice=sizeof($arr)-1;
									@$arr[$indice].=' Acuerdo Número'.$partes[0];
					...
					else
					{
						if(!isset($arr[0]))
						{
							if($conta==0)
							{
								$text1.= ' Acuerdo Número '.$tex_acta." ";	
							}
							else
							{
								$text1.= 'Comúniquese. Acuerdo Número '.$tex_acta." ";	
							}
							$conta+=1;
						...
						$indice=sizeof($arr)-1;
							...
							$arr[$indice].= 'Comúniquese. Acuerdo Número '.$tex_acta." ";
						}
...
$pdf=new PDFi('P','mm',array(216,320));
...
if($exten>0)
{
	...
	for($aa=0; $aa<=$exten;$aa++)
	{
		if($aa%2==0)
		{
			...
			$dato_add='Comúniquese. ';
			$sal_=mysql_query("select * from ob_01 where ob1_idact=$id");
				$sal_arr1=mysql_fetch_array($sal_);
			$dato_add.=$sal_arr1['ob1_text'];
			$sal_2=mysql_query("select * from ob_02 where ob2_idact=$id");
				$sal_arr2=mysql_fetch_array($sal_2);
				$dato_add.=$sal_arr2['ob2_text'];
			
			$sal_3=mysql_query("select * from ob_03 where ob3_idact=$id");
				$sal_arr3=mysql_fetch_array($sal_3);
				$dato_add.=$sal_arr3['ob3_text'];
			...
			if($aa==($salve_val-1))
			{
				...
				//fecha
					...
					$part_=explode('-',$dat);
								...
								// fecha letras año
					$fe_=convertir_a_letras($part_[0],false,'');
					$fecha_=str_replace('/-',$fe_,$fecha_);
			
						//fecha letras dia
						if($part_[2]<10)
						{
							$fe_=unidad($part[2],false,'');
						}
						elseif($part_[2]>=10)
						{
							$fe_=decena($part_[2],false,'');
						}
						$fecha_=str_replace('@',$fe_,$fecha_);
				...
				$pdf->MultiCell(155,6,$line.' '.utf8_decode($dato_add.'  Es conforme con su original con el cual se confrontó y para efectos de la Ley se expide la pressente en la oficina de  Secretaria Municipal de Usulután el dia '.$fecha_.'.'),0,'J');
			
			}
			else
			{
			$pdf->MultiCell(155,6,$line,0,'J');
			}
		}
		else
		{
	...
	$line=str_replace('?','"',$line);
				...
				//fecha
					...
					$fecha_=str_replace('/-',$fe_,$fecha_);
						...
						elseif($part_[2]>=10)
						{
							$fe=decena($part_[2],false,'');
						}
						$fecha_=str_replace('@',$fe,$fecha_);

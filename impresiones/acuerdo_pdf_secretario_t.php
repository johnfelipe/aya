...
$dato_acta_d=mysql_query("SELECT dato FROM actas WHERE act_id=$acta");
$dato_dato=mysql_fetch_array($dato_acta_d);

$datos_query=mysql_query("SELECT * FROM datos WHERE n_datos=$dato_dato[0]");
...
$row_acta=mysql_fetch_array($query_acta);
	...
	{
		$row_datos['acue_neck']=str_replace('==',' <modificar este dato y tambien en dato.acta_head2>  ',$row_datos['acue_neck']);/// `para las nuevos tipos de seciones
	}
...
*/
$query_sex_secre=mysql_query("SELECT personas.id_genero, personas.nombres, personas.primer_apellido, personas.segundo_apellido, cargo.car_m, cargo.car_f, profesiones.nomenclatura, profesiones.nomenclatura2 FROM personas, profesiones, cargo, nomina_consejo, relacion_actas, acuerdos WHERE personas.id_persona=nomina_consejo.nomina_perid AND cargo.rango=5 AND cargo.car_id=nomina_consejo.nomina_carid
AND nomina_consejo.nomina_id=relacion_actas.relacion_funcionario AND relacion_actas.relacion_actaid=acuerdos.acu_actid AND personas.id_profesion= profesiones.id_profesion  AND acuerdos.acu_id=$id_acu_pdf");
...
//echo $alcalde=$row_alcal['nombres']." ".$row_alcal['primer_apellido']." ".$row_alcal['segundo_apellido'];
	if($row_secre['id_genero']=='F')
	{
		...
		@$head='LA INFRASCRITA SECRETARIA MUNICIPAL,';	
	...
	else
	{
			...
			@$head='EL INFRASCRITO SECRETARIO MUNICIPAL,';		
...
$pdf->MultiCell(160,6,utf8_decode($head),0,'J');
$pdf->Ln(2);
/*

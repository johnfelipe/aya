	...
	for($aa=0; $aa<=$exten;$aa++)
	{
		if($aa%2==0)
		{
			...
			if($aa==($salve_val-1))
			{
				...
				$pdf->MultiCell(155,6,$line.' '.utf8_decode($dato_add.' Se cierra la presente y para constancia firmamos.'),0,'J');
...
$pdf->SetFont('times','',10);
	...
	while($pepe=mysql_fetch_array($query_asis))
	{
		...
		if(!($bb%2==0))
		{
			...
			else
			{
				$cordenadasY=$pdf->GetY();
				 ...
				 {
					$pdf->Ln(8);
					$pdf->multicell(70,3,utf8_decode($per)."\n".utf8_decode($genero),0,'C');
					$cordenadasY=$pdf->GetY();
				 }
				 else
				 {
		...
		else
		{
			...
			$pdf->multicell(60,3,utf8_decode($per)."\n".utf8_decode($genero),0,'C');
			$pdf->Ln(10);//lineas entre renglon de firmas mod 17/05/2012 14:54
	...
	if($cordenadas_final_Y >= 299)
	{
		$cordenadas_final_Y=25;
	}

	if($candado==1)
	{
		if(mysql_query("UPDATE actas SET act_Y=$cordenadas_final_Y WHERE act_id=$id"));
	}
